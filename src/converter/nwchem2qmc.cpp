...
//  cout<<"hello "<<basis[0].nfunc()<<endl;
  ...
  string orboutname=outputname+".nw.orb";
  slwriter.orbname=orboutname;
  string basisoutname=outputname+".nw.basis";
...
//slater output
  string slateroutname=outputname+".nw.slater";
  ofstream slaterout(slateroutname.c_str());
  slwriter.print_wavefunction(slaterout);
  slaterout.close();

  // jastrow 2 ouput
   string jast2outname=outputname+".nw.jast2";
  ...
  string jast3outname=outputname+".nw.jast3";
...
//system output
  string sysoutname=outputname+".nw.sys";
...
{  
   ...
   //   {  basis.size()=words[2]; }
      ...
      { getline(inFile,line);
        ...
        { slwriter.calctype="DFT_CLOSED"; }
        else if (words.size()==4 && words[2]=="spin"  && words[3]=="polarized.")
        { slwriter.calctype="DFT_OPEN";  }
