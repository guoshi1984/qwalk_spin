...
void write_basis(string oname, string twist,
		double* kx, double* ky, double* kz, double* k2, int* idx,
...
int main(int argc, char ** argv) {
	...
	for (int iK=0; iK<NKpt; iK++) {
		...
		int ikmax2=(2*ikmax-1)*(2*ikmax-1);
		double kx[ikmax2], ky[ikmax2], kz[ikmax2], k2[ikmax2];
		int idx[ikmax2], shells[ikmax2];
		...
		for ( int i=-ikmax+1; i<ikmax; i++) {
			for ( int j=-ikmax+1; j<ikmax; j++) {
				...
				ky[seq]=Ky+j*k0;
				kz[seq]=Kz;
				k2[seq]=kx[seq]*kx[seq]+ky[seq]*ky[seq];
				seq++;
			}
		}  
		...
		if ( seq != ikmax2 ) {
			...
			cerr << seq << " instead of " << ikmax2 << endl;
			exit(1);
		}
		...
		for ( seq=0; seq < ikmax2; seq++ ) {
			...
			if ( seq+1 < ikmax2 && k2[idx[seq+1]] > k2[idx[seq]] ) {
				if ( shell > 0 ) { 
		...
		write_basis(oname, twist.str(), kx, ky, kz, k2, idx, basissize, outprec);
	...
	double Ekin_ex=0;
	for (int i=0; i<2; i++) {
		double kF=sqrt(4*pi*Ne[i]/(L*L));
		Ekin_ex+=L*L*pow(kF,4.0)/(16*pi);
	}
	Ekin=Ekin/weight;
	cout << setprecision(8);
...
void write_basis(string oname, string twist,
	...
	basisout << "   SO_CPLANEWAVE" << endl; 
	basisout << "   GVECTOR {" << endl;

	//basisout << "   #  1 2 "<<endl;
	//basisout << "   ";
	//basisout << setprecision(outprec)  << setw(outprec+6) << kx[idx[0]] 
	//	<< setprecision(outprec)  << setw(outprec+6) <<  ky[idx[0]] 
	//	<< setprecision(outprec)  << setw(outprec+6) << 0.00000000000000 
	//	<< setprecision(outprec)  << setw(outprec+6) << 1.00000000000000 
	//	<< setprecision(outprec)  << setw(outprec+6) << 0.00000000000000
	//	<< setprecision(outprec)  << setw(outprec+6) << 1.00000000000000
	//	<< setprecision(outprec)  << setw(outprec+6) << 0.00000000000000
	//	<<endl;
	...
	for (int i=0; i < basissize; i++) {
		if(kx[idx[i]]==0&ky[idx[i]]==0) 
                {  basisout << "   #  1 2 "<<endl;
                   basisout << "   ";
                   basisout << setprecision(outprec)  << setw(8) << kx[idx[0]] <<"  " 
                            << setprecision(outprec)  << setw(8) <<  ky[idx[0]] <<"  "
        	            << setprecision(outprec)  << setw(8) << 0.00<<"  "
                            << setprecision(outprec)  << setw(8) << 1.00<<"  " 
                            << setprecision(outprec)  << setw(8) << 0.00 << "  "
                           << setprecision(outprec)  << setw(8) << 1.00 <<"  "
                            << setprecision(outprec)  << setw(8) << 0.00 <<"  "
                            << endl;
                  ...
                  basisout << setprecision(outprec)  << setw(8) << kx[idx[0]]<<"  "
                           << setprecision(outprec)  << setw(8) <<  ky[idx[0]] <<"  "
                           << setprecision(outprec)  << setw(8) << 0.000 <<"  "
                           << setprecision(outprec)  << setw(8) << -1.000 <<"  "
                           << setprecision(outprec)  << setw(8) << 0.000 << "  "
                           << setprecision(outprec)  << setw(8) << 1.0000 << "  "
                           << setprecision(outprec)  << setw(8) << 0.0000 << "  "
                           <<endl;

                } 
                else 
                {
                  basisout << "   #"<< 2*i+1 << "  " << 2*i+2 <<endl;
		  basisout << "   ";
		  basisout << setprecision(outprec) << setw(8) << kx[idx[i]]<<"  "
			<< setprecision(outprec) << setw(8) << ky[idx[i]]<<"  "
			<< setprecision(outprec) << setw(8) << kz[idx[i]]<<"  "
			<< setprecision(outprec) << setw(8) << ky[idx[i]]/sqrt(k2[idx[i]]) <<"  "
			<< setprecision(outprec) << setw(8) << kx[idx[i]]/sqrt(k2[idx[i]]) <<"  "
			<< setprecision(outprec) << setw(8) << 1.00  <<"  "
  			<< setprecision(outprec) << setw(8) << 0.00  <<"  "
...
void write_slater(string oname, string twist, int* Ne) {
	...
	//for(int k=0; k < 2; k++) {
	for (int orb=0; orb < Ne[0]; orb++) 
	{
		if((orb%10==0)&&(orb!=0)) slaterout<<endl; 
		slaterout << setw(6) << 2*orb+1;  
	}
	slaterout<<endl;
	for (int orb=0; orb < Ne[1]; orb++) 
	{ if((orb%10==0)&&(orb!=0)) slaterout<<endl;
		slaterout << setw(6) << 2*orb+2; 
...
void write_sys(string oname, string twist, int* Ne, double L,
	...
	sysout << "SYSTEM { SO_HEG" << endl;
	sysout << " NSPIN { " << Ne[0]+Ne[1] << " " << 0  << " }" << endl;
