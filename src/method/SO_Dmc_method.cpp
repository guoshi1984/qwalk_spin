...
void SO_Dmc_method::read(vector <string> words,
                      unsigned int & pos,
                      Program_options & options)
{
  ...
  if(haskeyword(words, pos=0, "COMPLEX")) {
    complex_weight=1;
    if(!pure_dmc)
      error("PURE_DMC has to be set for COMPLEX to be used");
  } 
  else
    complex_weight=0;
...
int SO_Dmc_method::generateVariables(Program_options & options) {
...
int SO_Dmc_method::allocateIntermediateVariables(System * sys,
...
int SO_Dmc_method::showinfo(ostream & os)
{
  ...
  os << "###########################################################\n";
  os << "Spin Orbit Diffusion Monte Carlo:\n";
  ...
  if(pure_dmc){
    os << "Branching is turned off"<<endl;
    os << "and correlation history lenght is  "<<nhist<<" steps."<<endl; 
    if(complex_weight)
      os << "In addition we calculate complex E_L and weights."<<endl;
  }
...
void SO_Dmc_method::find_cutoffs() {
  ...
  for(int i=0; i< nconfig; i++) {
    ...
    eaverage+=pts(i).pt.energy(0)*pts(i).weight;
  ...
  for(int i=0; i < nconfig; i++){
    doublevar effenergy=pts[i].pt.energy(0);
...
void SO_Dmc_method::run(Program_options & options, ostream & output) {
  if(!have_allocated_variables) 
 ...
 */
void SO_Dmc_method::runWithVariables(Properties_manager & prop, 
                                  System * sys, 
                                  Wavefunction_data * wfdata,
                                  ...
                                  ostream & output)
{
  ...
  if(complex_weight)
    prop.set_complex(); //has to be done before prop.setSize;
  ...
  */
  ...
  for(int block=0; block < nblock; block++) {
	...
	pts(walker).so_config_pos.restorePos(sample);
        pts(walker).so_config_pos.restoreOmega(sample);
	...
	//------Do several steps without branching
	  ...
	  if(complex_weight)
	    pt.is_complex=1;
          ...
          if(evaluate_so==1)
          { mygather.gatherDataWithSO(pt, pseudo, sys, wfdata, wf, sample, guidingwf);
          }
          else { 
	    mygather.gatherData(pt, pseudo, sys, wfdata, wf, 
          ...
          new_hist.main_en=pts(walker).pt.energy(0);
	  new_hist.is_complex=0;
	  if(complex_weight){
	    new_hist.is_complex=1;
	    new_hist.cmain_en=pts(walker).pt.cenergy(0);
	  }
	  ...
	  pts(walker).pt=pt;
	    ...
	    if(!complex_weight)
	      ...
	      assert(pts(walker).pt.is_complex==1);
	      pts(walker).cweight=getcWeightPURE_DMC(pts(walker),teff,etrial); // MB this is only used for avereraging (not for dynamics) and only if pure_dmc=1
	      pts(walker).weight=cabs(pts(walker).cweight);
	    }
	  }
          ...
          if(pts(walker).ignore_walker) {
            ...
            pts(walker).weight=1;
            pts(walker).pt.count=0;
          }
          pts(walker).pt.weight=pts(walker).weight;
	  if(complex_weight)
	    pts(walker).pt.cweight=pts(walker).cweight; // MB this is only used for avereraging (not for dynamics) and only if pure_dmc=1

          //This is somewhat inaccurate..will need to change it later
          //For the moment, the autocorrelation will be slightly
          //underestimated
          pts(walker).pt.parent=walker;
          pts(walker).pt.nchildren=1;
          pts(walker).pt.children(0)=walker;
          pts(walker).pt.avgrets.Resize(1,average_var.GetDim(0));
          for(int i=0; i< average_var.GetDim(0); i++) { 
            average_var(i)->evaluate(wfdata, wf, sys, sample, pts(walker).pt.avgrets(0,i));
          }


          prop.insertPoint(step+p, walker, pts(walker).pt); //Properties_point pts(walker).prop 
                                     //Properties_point: properties for each walker, at each step
          for(int i=0; i< densplt.GetDim(0); i++)
            densplt(i)->accumulate(sample,pts(walker).pt.weight(0));
          for(int i=0; i< nldensplt.GetDim(0); i++)
            nldensplt(i)->accumulate(sample,pts(walker).pt.weight(0),
                                     wfdata,wf);
        ...
        pts(walker).so_config_pos.savePos(sample);
        pts(walker).so_config_pos.saveOmega(sample);
      }// walkers
      ...
      //cout << "1111"<<block <<" "<< etrial <<endl; 
      updateEtrial(feedback);
      //cout<< "2222"<<block<<" "<<etrial <<endl; 
...
void SO_Dmc_method::savecheckpoint(string & filename,                     
                                 Sample_point * config) {
  if(filename=="") return;
...
void SO_Dmc_method::restorecheckpoint(string & filename, System * sys,
                                    Wavefunction_data * wfdata,
                                    Pseudopotential * pseudo) {
 ...
 */
  ...
  if(nconfig < ncread) { 
    Array1 <SO_Dmc_point> tmp_pts(nconfig);
  ...
  for(int walker=0; walker < nconfig; walker++) {
    ...
    pts(walker).so_config_pos.restoreOmega(sample);
    if(complex_weight){
      pts(walker).pt.is_complex=1;
    }

    mygather.gatherDataWithSO(pts(walker).pt, pseudo, sys,
                        wfdata, wf, sample,
                        guidingwf);
...
void SO_Dmc_method::cdmcReWeight(Array2 <doublevar> & energy_temp, 
...
void SO_Dmc_method::updateEtrial(doublevar feed) {
...
doublevar SO_Dmc_method::getWeightPURE_DMC(SO_Dmc_point & pt,
                                doublevar teff, doublevar etr) {
...
//----------------------------------------------------------------------
//using the pure DMC weight from last N steps in history complex return
dcomplex SO_Dmc_method::getcWeightPURE_DMC(SO_Dmc_point & pt,
					doublevar teff, doublevar etr) {
  doublevar teffac=teff;

  int history=pt.past_energies.size();
  assert(history>0);
  dcomplex sum=dcomplex(0.0,0.0);
  for(int i=0;i<history;i++){
    if(!pt.past_energies[i].is_complex)
      error("pt.past_energies.is_complex failed");
    sum+=pt.past_energies[i].cmain_en;
  }
  sum/=history;
  
  dcomplex return_weight;
  return_weight=exp(teffac*history*(etr-sum));
...
int SO_Dmc_method::calcBranch() { 
  ...
  Array1 <SO_Dmc_point> savepts=pts;
...
//----------------------------------------------------------------------
void SO_Dmc_point::mpiSend(int node) {
#ifdef USE_MPI
  pt.mpiSend(node);
  so_config_pos.mpiSend(node);
  ...
  MPI_Send(&pt.is_complex,1, MPI_INT,node,0,MPI_Comm_grp);
  if(pt.is_complex){
    MPI_Send(&cweight.real(),1,MPI_DOUBLE,node,0,MPI_Comm_grp);
    MPI_Send(&cweight.imag(),1,MPI_DOUBLE,node,0,MPI_Comm_grp);
  }
#endif
}
...
void SO_Dmc_point::mpiReceive(int node) {
#ifdef USE_MPI
  pt.mpiReceive(node);
  so_config_pos.mpiReceive(node);
  int n;
  MPI_Status status;
  ...
  MPI_Recv(&pt.is_complex,1,MPI_INT,node,0,MPI_Comm_grp, &status);
  if(pt.is_complex){
    MPI_Recv(&cweight.real(),1,MPI_DOUBLE,node,0,MPI_Comm_grp, &status);
    MPI_Recv(&cweight.imag(),1,MPI_DOUBLE,node,0,MPI_Comm_grp, &status);
  }
...
void SO_Dmc_point::write(ostream & os) {
  string indent="";
  so_config_pos.write(os);
  os << "DMC { \n";
...
void SO_Dmc_point::read(istream & is) {
  so_config_pos.read(is);
