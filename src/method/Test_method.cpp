...
void check_numbers(dcomplex num1,dcomplex num2, ostream & os,
		   doublevar tol=1e-5) { 
  string ok="OK";
  
  //make sure you are not divinding by zero
  //doublevar tol2=tol*tol;
  doublevar norm=cabs(num2);
  if(norm<tol){
    if(cabs(num1)<tol)
      norm=1.0;
    else
  ...
  dcomplex diff=(num1-num2)/norm;
  if( cabs(diff) > tol) ok="FAILED";
  os << num1 << "    " << num2 <<  "   " << ok << endl;
}
  ...
  */
  ...
  for(int e=0; e < nelectrons; e++) {
    ...
    for(int d=0;d < 3; d++) {
      ...
      //to test the >updateVal/getVal routines
...
void Test_method::testParmDeriv(Wavefunction * mywf, Sample_point * sample){
  ...
  //if complex wfders.is_complex=1
  if(wfders.is_complex){
    cout <<" detected complex derivatives "<<endl;
  }
  dcomplex psi_complex,derivative_complex, hessian_wf_complex; 
  ...
  for (int i=0;i<nparms;i++){
    ...
    if(wfders.is_complex){
      psi_complex=exp(wfval.amp(0,0)-test_wfval.amp(0,0))*exp(I*(wfval.phase(0,0)-test_wfval.phase(0,0)));
      derivative_complex=(psi_complex-1.0)/del;
    }
    Psi(i)=psi;
    ...
    if(!wfders.is_complex)
      ...
      check_numbers(derivative_complex,wfders.cgradient(i),cout);
    }
	...
	if(!wfders.is_complex)
	  hessian_wf=(psi*wfders_tmp.gradient(j)-wfders.gradient(j))/del;
	else
	  hessian_wf_complex=(psi_complex*wfders_tmp.cgradient(j)-wfders.cgradient(j))/del;

	if(!wfders.is_complex)
	  check_numbers(hessian_wf,wfders.hessian(i,j),cout);
	else{
	  check_numbers(hessian_wf_complex,wfders.chessian(i,j),cout);
	}
      }
    }
