...
#include "twoD_SO_4D_Split_sampler.h"
 ...
 */
doublevar twoD_SO_4D_Split_sampler::transition_prob(int point1, int point2,
                                         doublevar timestep, 
                                         drift_type dtype) {
  doublevar prob=0;
...
void twoD_SO_4D_Split_sampler::read(vector <string> & words) {
  unsigned int pos=0;
 ...
 */
doublevar twoD_SO_4D_Split_sampler::get_acceptance(Guiding_function * guidingwf
                                        ) {
  //indent = indent + "  ";
...
void twoD_SO_4D_Split_sampler::Norm(Array1 <doublevar> &omega)
{     
   doublevar rr=0;
      for(int i=0;i<4;i++)
        rr+=omega[i]*omega[i];
      rr=sqrt(rr);
      for(int i=0;i<4;i++)
        omega[i]=omega[i]/rr;   //normalized
}

//----------------------------------------------------------------------
int twoD_SO_4D_Split_sampler::split_driver(int e,
                                Sample_point * sample,
                                Wavefunction * wf, 
                                Wavefunction_data * wfdata,
                                Guiding_function * guidingwf,
                                int depth,
                                Dynamics_info & info,
                                doublevar & efftimestep, doublevar & spintimestep
                                ) 
{
...
//    trace(depth).pos(d)=trace(0).pos(d)
 ...
 doublevar step=sqrt(spintimestep)*rng.gasdev();
 Array1 <doublevar> random;
 random.Resize(4);
 for(int i=0;i<4;i++)       random[i]=2*rng.gasdev()-1;
 Norm(random);
 doublevar thet=acos(trace(0).omega[0]*random[0]
               +trace(0).omega[1]*random[1]
               +trace(0).omega[2]*random[2]
               +trace(0).omega[3]*random[3]);

 doublevar alpha=pi-(pi-thet)/2-step;
 doublevar l=sin(step)/sin(alpha);
 Array1 <doublevar> vec;
 vec.Resize(4);
 for(int i=0;i<4;i++) vec[i]=trace(0).omega[i]-random[i];
 Norm(vec);
 for(int i=0;i<4;i++) trace(depth).translation[i+3]=vec[i]*l;
 //Norm(trace(depth).omega);
  ...
  */
  ...
  if (acc+rng.ulec() > 1.0) {
    ...
    Array1 <doublevar> rev(4,0.0);
    for(int d=0; d< 4; d++) rev(d)=-trace(depth).translation(d);
...
int twoD_SO_4D_Split_sampler::sample(int e,
                          Sample_point * sample, 
                          Wavefunction * wf, 
                          Wavefunction_data * wfdata,
                          Guiding_function * guidingwf,
...
void twoD_SO_4D_Split_sampler::showStats(ostream & os) {
  doublevar totacc=0;
  for(int i=0; i< recursion_depth_; i++) {
...
void twoD_SO_4D_Split_sampler::resetStats() {
  acceptances=0;
  tries=0;
}
