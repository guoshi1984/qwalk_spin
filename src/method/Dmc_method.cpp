	...
	//------Do several steps without branching
	  ...
	  if(complex_weight)
	    ...
	    if(!complex_weight)
	      ...
	      sys->calcKinetic(wfdata,sample,wf,pt.ckinetic);
	      for(int f=0;f<wf->nfunc();f++)
		pt.kinetic(f)=pt.ckinetic(f).real();
	    }
	  ...
	  if(complex_weight){
	    ...
	    new_hist.cmain_en=pts(walker).prop.cenergy(0);
	  }
	  ...
	  pts(walker).prop=pt;
	    ...
	    if(!complex_weight)
	      ...
	      assert(pts(walker).prop.is_complex==1);
	  ...
	  if(complex_weight)
	    pts(walker).prop.cweight=pts(walker).cweight; // MB this is only used for avereraging (not for dynamics) and only if pure_dmc=1
...
dcomplex Dmc_method::getcWeightPURE_DMC(Dmc_point & pt,
					doublevar teff, doublevar etr) {
  doublevar teffac=teff;
...
void Dmc_point::mpiSend(int node) { 
  ...
  MPI_Send(&prop.is_complex,1, MPI_INT,node,0,MPI_Comm_grp);
  if(prop.is_complex){
...
void Dmc_point::mpiReceive(int node) {
  ...
  MPI_Recv(&prop.is_complex,1,MPI_INT,node,0,MPI_Comm_grp, &status);
  if(prop.is_complex){
...
void Dmc_history::mpiSend(int node) { 
  ...
  if(is_complex){
    MPI_Send(&cmain_en.real(),1,MPI_DOUBLE,node,0,MPI_Comm_grp);
    MPI_Send(&cmain_en.imag(),1,MPI_DOUBLE,node,0,MPI_Comm_grp);
  }
...
void Dmc_history::mpiReceive(int node) { 
  ...
  if(is_complex){
    MPI_Recv(&cmain_en.real(),1,MPI_DOUBLE,node,0,MPI_Comm_grp,&status);
    MPI_Recv(&cmain_en.imag(),1,MPI_DOUBLE,node,0,MPI_Comm_grp,&status);
  }
