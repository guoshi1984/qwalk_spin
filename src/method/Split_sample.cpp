...
int allocate(vector <string> & words, Dynamics_generator *& sam) {
  if(caseless_eq(words[0],"SPLIT"))
    ...
    sam=new UNR_sampler;
  else if(caseless_eq(words[0],"SO_SPLIT"))
    sam=new SO_Split_sampler;  
  else if(caseless_eq(words[0],"2D_SO_SPLIT"))
    sam=new twoD_SO_Split_sampler; 
  else if(caseless_eq(words[0],"2D_SPLIT")) 
    sam=new twoD_Split_sampler;
  else 
