 ...
 */
doublevar SO_Split_sampler::transition_prob(int point1, int point2,
                                         doublevar timestep, 
                                         drift_type dtype) {
  doublevar prob=0;
...
void SO_Split_sampler::read(vector <string> & words) {
  unsigned int pos=0;
 ...
 */
doublevar SO_Split_sampler::get_acceptance(Guiding_function * guidingwf
                                        ) {
  //indent = indent + "  ";
...
//----------------------------------------------------------------------
int SO_Split_sampler::split_driver(int e,
                                Sample_point * sample,
                                Wavefunction * wf, 
                                Wavefunction_data * wfdata,
                                Guiding_function * guidingwf,
...
//    trace(depth).pos(d)=trace(0).pos(d)
 ...
 // cout<<"trace before"<<trace(0).pos(2)<<" after "<<trace(1).pos(2)<<endl;  
  ...
  //cout<<" lap " <<trace(1).lap.amp(0,0) <<endl;
...
int SO_Split_sampler::sample(int e,
                          Sample_point * sample, 
                          Wavefunction * wf, 
                          Wavefunction_data * wfdata,
                          Guiding_function * guidingwf,
...
void SO_Split_sampler::showStats(ostream & os) {
  doublevar totacc=0;
  for(int i=0; i< recursion_depth_; i++) {
...
void SO_Split_sampler::resetStats() {
  acceptances=0;
  tries=0;
}
