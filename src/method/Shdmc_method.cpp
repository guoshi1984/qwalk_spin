                           ...
                           Program_options & options)
{
  ...
  if(!readvalue(words, pos=0, scale_step, "SCALE_DLAMBDA") )
  {
   ...
   if(!readvalue(words, pos=0, twice, "NOISE_BARRIER") )
  {
    twice=2.0;
  }
...
void Shdmc_method::get_averages(Array1 <doublevar> & parms,
  ...
  for (int mc_size=0; mc_size<mc_words.size(); mc_size++){
    ...
    int complex_weight=0;
    unsigned int pos;
    if(haskeyword(run_words, pos=0, "COMPLEX")) {
      complex_weight=1;
      single_write(cout, "SHDMC with complex weights detected \n");
    } 
	...
	if(!complex_weight)
	  for(int j=0; j< ndim; j++) { 
	    ...
	    if(fabs(parms(j))>0)
	      ...
	      delta_parms(j)=0.0; 
	    doublevar error;
	    error=finavg.avgerr(0,i).vals(j)/finavg.avgavg(0,i).vals(ndim);
	    //output << value << " +/- "<<error<<endl;
	    if(abs(value)>twice*error){
	      new_parms(j)=value;
	    }
	  ...
	  //single_write(cout,"adding complex linear_delta_der\n");
	  if(ndim%2!=0)
	    error("SHDMC with complex linear_delta_der should have even number of parameters.");
	  int mdim=int(ndim/2);
	  Array1 <doublevar> errors(mdim);
	  Array1 <doublevar> new_values(mdim);

	  for(int j=0; j< mdim; j++) {
	    doublevar ea=scale_step*finavg.avgerr(0,i).vals(2*j)/finavg.avgavg(0,i).vals(ndim);
	    doublevar eb=scale_step*finavg.avgerr(0,i).vals(2*j+1)/finavg.avgavg(0,i).vals(ndim);
	    doublevar da=scale_step*finavg.avgavg(0,i).vals(2*j)/finavg.avgavg(0,i).vals(ndim);
	    doublevar db=scale_step*finavg.avgavg(0,i).vals(2*j+1)/finavg.avgavg(0,i).vals(ndim);
	    doublevar va=parms(2*j)+da;
	    doublevar vb=parms(2*j+1)+db;
	    if(sqrt(parms(2*j)*parms(2*j)+parms(2*j+1)*parms(2*j+1))>0){
	      delta_parms(2*j)=da;
	      delta_parms(2*j+1)=db;
	    }
	    else{
	      delta_parms(2*j)=0.0;
	      delta_parms(2*j+1)=0.0;
	    }

	    errors(j)=sqrt(ea*ea+eb*eb);
	    new_values(j)=sqrt(va*va+vb*vb);
	    single_write(cout,"cabs(new_parm) = ", new_values(j));
	    single_write(cout," +/- ", errors(j));
	    single_write(cout,"\n");
	    if(new_values(j)> twice*errors(j)){
	      new_parms(2*j)=va;
	      new_parms(2*j+1)=vb;
	    }
	    else{
...
int Shdmc_method::showinfo(ostream & os)
{
  ...
  os << "NOISE_BARRIER "<<twice<<endl;
  os << "SCALE_DLAMBDA "<<scale_step<<endl;
  os << "----------------------------" << endl;
  return 1;
}
...
void Shdmc_method::run(Program_options & options, ostream & output)
{
  ...
  while(iter<iterations && !reached_max_blocks){
    ...
    */
    ...
    if(norm_old >0 && norm_new >0)
      scalar_product_of_delta_lamda/=sqrt(norm_old*norm_new);
    else
      scalar_product_of_delta_lamda=0.0;
	...
	single_write(cout,"scalar_product_of_delta_lamda>0,  adding extra block\n");
	nblocks+=1;
      }
      else{
	nblocks=int(block_increase_factor*nblocks);
	single_write(cout,"scalar_product_of_delta_lamda<=0, increasing NBLOCKS to ",nblocks);
	single_write(cout,"\n");
      }
      if(nblocks>nblocks_max)
	nblocks=nblocks_max;
