...
#include "Cutoff_cusp_2D.h"
#include "qmc_io.h"

/*
 
*/
int Cutoff_cusp_2D::read(
  vector <string> & words,
  unsigned int & pos)
{
...
void Cutoff_cusp_2D::getVarParms(Array1 <doublevar> & parms) {
...
void Cutoff_cusp_2D::setVarParms(Array1 <doublevar> & parms) {
...
int Cutoff_cusp_2D::nfunc()
{
  return 1;
}

int Cutoff_cusp_2D::showinfo(string & indent, ostream & os)
{
...
int Cutoff_cusp_2D::writeinput(string & indent, ostream & os)
{
...
void Cutoff_cusp_2D::raw_input(ifstream & input)
{error("Raw input not supported by Cutoff_cusp");}

void Cutoff_cusp_2D::calcVal(const Array1 <doublevar> & r,
...
//change from 3d to 2d
  ...
  */
   ...
   doublevar dadr2=ppdd*pade2*rcutinv+dadr
                      -2.*gamma*ppd*ppd*pade2*pade*rcutinv;
    for(int i=1; i< 4; i++) {
