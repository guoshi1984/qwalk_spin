                           ...
                           const int startfill)
{
  if(r(0) >= threshold)
    ...
    if(!recursive){ //original approach
	...
	int interval=splines(i).getInterval(r(0));
	temp=splines(i).getVal(r(0), interval);
	int l=symmetry_lvalue(symmetry(i));
	for(int m=0;m<=l;m++){ //starting with m=0 first
	  if(m>=0)
	    symvals(totf++)=temp*realYlm->getYlm(l, m);
	  if(m>0)
	    symvals(totf++)=temp*realYlm->getYlm(l, -m); 
	}
  ...
  else
  {
	...
	case isym_G8: //g8
	  ...
	  break;
       ...
       register doublevar func, fdir, f2dir;
       register doublevar h, x, y, z, v4, dfdx, dfdy, dfdz;
       
       assert(r(0) < threshold);
       assert(symvals.GetDim(0) >= nfunctions);

       realYlm->evaluateAll(r);
	 ...
	 int interval=splines(i).getInterval(r(0));
	 splines(i).getDers(r(0), interval,func, fdir, f2dir);
	 //cout << "recursive: rad fn " << func << "   " << fdir << "   "
	 //  << f2dir << endl;
	 int l=symmetry_lvalue(symmetry(i));
	 for(int m=0;m<=l;m++){
	   if(m>=0){
	      v4=realYlm->getYlm(l, m);
	      realYlm->getGradYlm(l,m,gYlm);
	      dfdx=gYlm(0);
	      dfdy=gYlm(1);
	      dfdz=gYlm(2);
	      h=fdir*v4;
   ...
   if(!recursive){ //original approach
	...
	case isym_G0:
	  ...
	  *(ptr++)=func*d2gdydz+z*fdir*dgdy+y*fdir*dgdz+v4*(y*z/v5)*(f2dir-fdir);
	  ...
	  d2gdydy=12.*v2 - 12.*v3;
	  d2gdzdz=12.*v1 - 12.*v2;
	  cout <<"non recursive d2gdzdz "<<d2gdzdz<<endl;
	  d2gdxdy=0;
	  d2gdxdz=24.*x*z;
	  d2gdydz=-24.*y*z;
	...
	case isym_G8:
     ...
     //this section has yet to be completed. M.B.
     register doublevar func, fdir, f2dir;
     register doublevar h, x, y, z, dfdx, dfdy, dfdz;
     register doublevar v1, v2, v3, v4, v5;
     register doublevar dgdx, dgdy, dgdz, d2gdxdx, d2gdydy, d2gdzdz, d2gdxdy, d2gdxdz, d2gdydz;
     ...
     realYlm->evaluateincludingHessian(r);
     Array1 <doublevar> gYlm(3);
     Array2 <doublevar> hYlm(3,3);
	 ...
	 for(int m=0;m<=l;m++){
	   if(m>=0){
	      ...
	      symvals(totf,0)=v4*func;
	      symvals(totf,1)=func*dgdx+h*x;
	      symvals(totf,2)=func*dgdy+h*y;
	      symvals(totf,2)=func*dgdz+h*z;
	      symvals(totf,4)=func*d2gdxdx+2*fdir*dgdx*x+v4*fdir+v4*(v1/v5)*(f2dir-fdir);
	      symvals(totf,5)=func*d2gdydy+2*fdir*dgdy*y+v4*fdir+v4*(v2/v5)*(f2dir-fdir);
	      symvals(totf,6)=func*d2gdzdz+2*fdir*dgdz*z+v4*fdir+v4*(v3/v5)*(f2dir-fdir);
	      symvals(totf,7)=func*d2gdxdy+y*fdir*dgdx+x*fdir*dgdy+v4*(x*y/v5)*(f2dir-fdir);
	      symvals(totf,8)=func*d2gdxdz+z*fdir*dgdx+x*fdir*dgdz+v4*(x*z/v5)*(f2dir-fdir);
	      symvals(totf,9)=func*d2gdydz+z*fdir*dgdy+y*fdir*dgdz+v4*(y*z/v5)*(f2dir-fdir);
	      totf++;
	   }
	   if (m>0){
	     v4=realYlm->getYlm(l, -m);
