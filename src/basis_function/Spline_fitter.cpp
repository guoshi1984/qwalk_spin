...
doublevar Spline_fitter::findCutoff() {
  const doublevar step=spacing;//0.5 this assures more accurate cutoff
  ...
  for(doublevar x=threshold-step; x >=0; x-=step) {
    ...
    if(fabs(test) > cutoff_threshold) {
      ...
      coeff.Resize(interval+2,4); //the rest of coeff will be all  zeroes
      return x+step;
    }
  }
  return 0;
