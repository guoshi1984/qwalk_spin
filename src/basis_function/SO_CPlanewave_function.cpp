...
/*!
   ...
   constraints..
*/
int SO_CPlanewave_function::read(
  vector <string> & words,
  unsigned int & pos)
{
  ...
  for(int i=0; i< nmax; i++) {
    ...
    for(int s=0; s< 2; s++)
    {
      doublevar real=atof(gvectxt[counter].c_str());
      //spinor(i,s).real()=atof(gvectxt[counter].c_str());
      counter++;
      doublevar imag=atof(gvectxt[counter].c_str());
      
      //spinor(i,s).imag()=atof(gvectxt[counter].c_str());
      counter++;
      dcomplex tmp(real, imag);
      spinor(i,s) = tmp;
    }
  }
  //cout << "done " << endl;
...
void SO_CPlanewave_function::getVarParms(Array1 <doublevar> & parms) {
  //cout << "getVarParms " << endl;
  parms.Resize(0);
}

void SO_CPlanewave_function::setVarParms(Array1 <doublevar> & parms) {
  //cout << "setVArParms " << endl;
  assert(parms.GetDim(0)==0);
}

int SO_CPlanewave_function::nfunc()
{
  //cout << "Planewave::nfunc() " << nmax << endl;
  return nmax;
}

int SO_CPlanewave_function::showinfo(string & indent, ostream & os)
{
...
int SO_CPlanewave_function::writeinput(string & indent, ostream & os)
{
...
void SO_CPlanewave_function::calcVal(const Array1 <doublevar> & r,
                                 ...
                                 const int startfill)
{
  ...
  for(int i=0; i< nmax; i++) {
    ...
    symvals(index,0)=exp(I*gdotr)*spinor(i,0)*exp(I*omega);
    symvals(index,1)=exp(I*gdotr)*spinor(i,1)*exp(-I*omega); 
    index++;
  }
  //cout << "done" << endl;
}

void SO_CPlanewave_function::calcSpinProjVal(const Array1 <doublevar> & r,
                                             const doublevar & omega,
                                             Array2 <dcomplex> & symvals,
                                             const int startfill )
{ 
  ...
  for(int i=0; i< nmax; i++) {
    ...
    symvals(index,0)=exp(I*gdotr)*spinor(i,0)*exp(I*omega);
    symvals(index,1)=exp(I*gdotr)*spinor(i,1)*exp(-I*omega);
    symvals(index,2)=exp(I*gdotr)*spinor(i,1)*exp(I*omega);
    symvals(index,3)=exp(I*gdotr)*spinor(i,0)*exp(-I*omega);
    index++;
  }
...
void SO_CPlanewave_function::calcLap(
...
void SO_CPlanewave_function::calcSpinOrbit(
  const Array1 <doublevar> & r,
  const doublevar & omega,
...
)
{
  ...
  //symvals(index, 0)=t_exp*(spinor(index,1)*exp(I*omega)
    //                      +spinor(index,0)*exp(-I*omega));
      symvals(index, 0)=g_vector(fn, 1)*t_exp*(spinor(index,1)*exp(I*omega)
                          +spinor(index,0)*exp(-I*omega));
      
//      symvals(index,1)=-t_exp*(spinor(index,0)*exp(I*omega)+
  //                              spinor(index,1)*exp(-I*omega));
      symvals(index, 1)=-g_vector(fn, 0)*t_exp*(-I*spinor(index,1)*exp(I*omega)
                       +I*spinor(index,0)*exp(-I*omega));
                        ;
//    symvals(index, 4, 0)=I*t_exp*spinor(index,0);
//    symvals(index, 4, 1)=I*t_exp*spinor(index,1);
    //cout<<" symvals "<< symvals(index,4,0)<<endl;
...
void SO_CPlanewave_function::debug( const Array1 <doublevar> & r, const doublevar & omega,Array3 <dcomplex> & symvals, const int startfill ) 
