...
int allocate(vector <string> & basistext, Basis_function * & bptr)
{
  ...
  else if(caseless_eq(type,"POLYPADE_2D"))
    bptr= new Poly_pade_function_2D;
  ...
  else if(caseless_eq(type,"CUTOFF_CUSP_2D"))
    bptr=new Cutoff_cusp_2D;
  ...
  else if(caseless_eq(type,"YUKAWA"))
    bptr=new Yukawa;
