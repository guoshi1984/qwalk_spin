...
Cubic_spline::symmetry_type Cubic_spline::symmetry_lookup(string & s) { 
  ...
  if(caseless_eq(s,"7F_siesta"))
    return sym_F_siesta; 
  if(caseless_eq(s,"9G_siesta")){
    return sym_G_siesta; 
  }
  if( caseless_eq(s,"H") || caseless_eq(s,"11H") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for H functions to work");
    return sym_H;
  }
  if( caseless_eq(s,"I") || caseless_eq(s,"13I") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for I functions to work");
    return sym_I;
  }
  if( caseless_eq(s,"K") || caseless_eq(s,"15K") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for K functions to work");
    return sym_K;
  }
  if( caseless_eq(s,"L") || caseless_eq(s,"17L") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for L functions to work");
    return sym_L;
  }
  if( caseless_eq(s,"M") || caseless_eq(s,"19M") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for M functions to work");
    return sym_M;
  }
  if( caseless_eq(s,"N") || caseless_eq(s,"21N") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for N functions to work");
    return sym_N;
  }
  if( caseless_eq(s,"O") || caseless_eq(s,"23O") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for O functions to work");
    return sym_O;
  }
  if( caseless_eq(s,"Q") || caseless_eq(s,"25Q") ){
    if(!recursive)
      error("Set RECURSIVE_EVAL for Q functions to work");
    return sym_Q;
  }
...
)
{
  ...
  recursive=false;
  addsign=true;
  if(haskeyword(words, pos=0, "RECURSIVE_EVAL")) 
    {recursive=true; }

  if(haskeyword(words, pos=0, "SIESTA_SIGN"))
    addsign=false;
...
int Cubic_spline::readspline(vector <string> & words) {
  ...
  if(recursive){
    realYlm= new RealSphericalTensor;
    Lmax=0;
    for(int i=0; i< nsplines; i++) {
      if(symmetry_lvalue(symmetry(i)) >Lmax)
	Lmax=symmetry_lvalue(symmetry(i));
    }
    realYlm->init(Lmax,addsign);
  }
...
int Cubic_spline::readbasis(vector <string> & words,unsigned int & pos,
  ...
  for(int funcNum=0; funcNum<nsplines; funcNum++)  {
    ...
    if(enforce_cusp) { //inspired from J. Chem. Phys. 130, 114107 (2009)
      ...
      double lndr=yp1/(y(1));
      single_write(cout, "enforcing cusp condition : original dir ", lndr, "  new ", der);
      single_write(cout, "\n");
      
      yp1=der*y(0); //y(0) added by MB so the cusp is done the same way as in Spline_fitter.cpp
...
void Cubic_spline::assign_indiv_symmetries() {
  ...
  for(int funcNum=0; funcNum < nsplines; funcNum++){
	...
	indiv_symmetry(totfunc++)=isym_Fp2;
     ...
     case sym_G_siesta:
        nfuncspline(funcNum)=9;
	//do not need indiv_symmetry for recursive evaluation
	break;
     case sym_H:
      	nfuncspline(funcNum)=11;
	//do not need indiv_symmetry for recursive evaluation
	break;
     case sym_I:
        nfuncspline(funcNum)=13;
        //do not need indiv_symmetry for recursive evaluation
        break;
     case sym_K:
        nfuncspline(funcNum)=15;
	//do not need indiv_symmetry for recursive evaluation
        break;
     case sym_L: 
        nfuncspline(funcNum)=17;
	//do not need indiv_symmetry for recursive evaluation
        break;
     case sym_M: 
        nfuncspline(funcNum)=19;
	//do not need indiv_symmetry for recursive evaluation
        break;
     case sym_N: 
        nfuncspline(funcNum)=21;
	//do not need indiv_symmetry for recursive evaluation
        break;
     case sym_O: 
        nfuncspline(funcNum)=23;
	//do not need indiv_symmetry for recursive evaluation
        break;
     case sym_Q: 
        nfuncspline(funcNum)=25;
	//do not need indiv_symmetry for recursive evaluation
        break;
      default:
...
int Cubic_spline::showinfo(string & indent, ostream & os)
{
  ...
  if(recursive){
    os << indent << "Using recursive evaluation of real spherical harmonics (addapted from Jeognim Kim's QMCPACK) upto Lmax "<<Lmax<<endl;
    if(addsign)
      os << indent << "with GAUSSIAN/GAMESS sign convention"<<endl;
    else
      os << indent << "with SIESTA sign convention"<<endl;
  }
  int totfunc=0;
  for(int i=0; i< nsplines; i++) {
