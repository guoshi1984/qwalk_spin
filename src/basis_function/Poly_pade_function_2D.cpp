...
#include "Poly_pade_function_2D.h"
#include "qmc_io.h"

/*
 
*/
int Poly_pade_function_2D::read(
  vector <string> & words,
  unsigned int & pos
)
{
...
void Poly_pade_function_2D::getVarParms(Array1 <doublevar> & parms) {
  parms.Resize(1);
...
void Poly_pade_function_2D::setVarParms(Array1 <doublevar> & parms) {
...
int Poly_pade_function_2D::nfunc()
{
  return nmax;
}

int Poly_pade_function_2D::showinfo(string & indent, ostream & os)
{
  os << indent << "Poly_pade function_2D\n";
  os << indent << "beta : ";
  for(int i=0; i< nmax; i++) 
    os << beta(i) << "  ";
  os << endl;
...
int Poly_pade_function_2D::writeinput(string & indent, ostream & os)
{
...
void Poly_pade_function_2D::raw_input(ifstream & input)
{
  error("raw_input doesn't work for Poly_pade function");
}

void Poly_pade_function_2D::calcVal(const Array1 <doublevar> & r,
...
void Poly_pade_function_2D::calcLap(
