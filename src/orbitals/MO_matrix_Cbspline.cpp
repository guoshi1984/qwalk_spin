...
)
{
  ...
  for(int m=0; m < nmo_list; m++) {
    ...
    for(int d=0;d<3;d++)
      newvals(m,d+1)= magnification_factor*multi_grad(mo)(d);
    int d=4;
    newvals(m,d++)=magnification_factor*multi_hess(mo)(0,0);
    newvals(m,d++)=magnification_factor*multi_hess(mo)(1,1);
    newvals(m,d++)=magnification_factor*multi_hess(mo)(2,2);
    newvals(m,d++)=magnification_factor*multi_hess(mo)(0,1);
    newvals(m,d++)=magnification_factor*multi_hess(mo)(0,2);
    newvals(m,d++)=magnification_factor*multi_hess(mo)(1,2);
  }//m
