...
#include "Qmc_std.h"
#include "MO_matrix_SO_Cbasfunc.h"
#include "Sample_point.h"
#include "qmc_io.h"

//--------------------------------------------------------------------------

void MO_matrix_SO_Cbasfunc::init() {
...
void MO_matrix_SO_Cbasfunc::buildLists(Array1 < Array1 <int> > & occupations) {
...
int MO_matrix_SO_Cbasfunc::showinfo(ostream & os)
{
...
int MO_matrix_SO_Cbasfunc::writeinput(string & indent, ostream & os)
{
...
*/
void MO_matrix_SO_Cbasfunc::read(vector <string> & words,
...
void MO_matrix_SO_Cbasfunc::updateVal(Sample_point * sample, int e,
                                   int listnum,
                                   //!< which list to use
  ...
  ) 
{
  ...
  Array2 <dcomplex> basisvals(totbasis,2);
  centers.updateDistance(e, sample);
  CBasis_function * tempbasis;
  Array1 <doublevar> R(5);
  doublevar omega;
  sample->getElectronOmega(e,omega);
  int currfunc=0;
  newvals=0;
  ...
  {  basisvals(i,0) = dcomplex(0.0, 0.0); 
     basisvals(i,1) = dcomplex(0.0, 0.0);
  ...
  //ion: index for different type of ions
  for(int ion=0; ion < centers.equiv_centers.GetDim(0); ion++) {
  //center: index for ions of the same type
    for(int c=0; c < centers.ncenters_atom(ion); c++) {
  //cen2: total ion index
      int cen2=centers.equiv_centers(ion, c);
      centers.getDistance(e, cen2, R);
      sample->getElectronOmega(e,omega);
      for(int n=0; n < centers.nbasis(cen2); n++) {
	int b=centers.basis(cen2,n);
  //b: basis index in each ion(same and diff type)
	tempbasis=basis(b);
	if(R(0) <obj_cutoff(b)) {
	  tempbasis->calcVal(R,omega, basisvals, currfunc);
	  ...
	  for(int m=0; m < nmo_list; m++) {
	    ...
	    if(eq_centers(mo)==ion){
		 ...
		 newvals(m,0,0)+=magnification_factor*basisvals(basfunc,0)*kptfac(cen2);
                 newvals(m,0,1)+=magnification_factor*basisvals(basfunc,1)*kptfac(cen2);
...
//------------------------------------------------------------------------
void MO_matrix_SO_Cbasfunc::updateSpinProjVal(Sample_point * sample, int e,
                                   int listnum,
                                   //!< which list to use
  ...
  ) 
{
  ...
  Array2 <dcomplex> basisvals(totbasis,4);
  centers.updateDistance(e, sample);
  CBasis_function * tempbasis;
  Array1 <doublevar> R(5);
  doublevar omega;
  ...
  for(int ion=0; ion < centers.equiv_centers.GetDim(0); ion++) {
	...
	tempbasis=basis(b);
	if(R(0) <obj_cutoff(b)) {
	  tempbasis->calcSpinProjVal(R,omega, basisvals, currfunc);
	  currfunc+=tempbasis->nfunc();
	  for(int m=0; m < nmo_list; m++) {
	    int mo=moLists(listnum)(m);
	    if(eq_centers(mo)==ion){
	       ...
	       if(R(0) < cutoff(basfunc)){
		 for(int j=0; j<4; j++)
                 { newvals(m,j)+=magnification_factor*basisvals(basfunc,j)*kptfac(cen2);
                 }
...
void MO_matrix_SO_Cbasfunc::updateLap(Sample_point * sample, int e,
				  int listnum,
				  //!< which list to use
				  Array3 <dcomplex> & newvals
  ...
  Array3 <dcomplex> basisvals(totbasis,5,2);
  centers.updateDistance(e, sample);
  CBasis_function * tempbasis;
  Array1 <doublevar> R(5);
  double omega;
  ...
  { for (int j=0; j< 5; j++)
    {  
       basisvals(i,j,0) =dcomplex(0.0, 0.0);
       basisvals(i,j,1) =dcomplex(0.0, 0.0);
    } 
  }
  ...
  for(int ion=0; ion < centers.equiv_centers.GetDim(0); ion++) {
	...
	if(R(0)< obj_cutoff(b)) {
     //     cout<<"R "<<"omega "<<R(2)<<" "<<omega<<endl;              
 	 // tempbasis->debug(R,omega, basisvals , currfunc); 
          tempbasis->calcLap(R, omega, basisvals, currfunc);
	  ...
	  for(int m=0; m < nmo_list; m++) {
	    ...
	    if(eq_centers(mo)==ion){
	      ...
	      if(R(0) < cutoff(basfunc)){
		for (int d=0; d < 5; d++) {
		  newvals(m,d,0)+=magnification_factor*basisvals(basfunc,d,0)*kptfac(cen2);
              //    cout<<"hello "<<basisvals(basfunc,d,0)<<" "<<m<<" "<< d<<newvals(m,d,0)<<endl;
                  newvals(m,d,1)+=magnification_factor*basisvals(basfunc,d,1)*kptfac(cen2);
		}
	      }
	    }
	  ...
	  currfunc+=tempbasis->nfunc();
       ...
       // cout<<"newvals "<<newvals(2,4,0)<<endl;
...
void MO_matrix_SO_Cbasfunc::updateSpinOrbit(Sample_point * sample, int e,
				  int listnum,
				  //!< which list to use
				  Array2 <dcomplex> & newvals
				  //!< The return: in form (MO, [val, grad, lap])
  ...
  for(int ion=0; ion < centers.equiv_centers.GetDim(0); ion++) {
	...
	if(R(0)< obj_cutoff(b)) {
     //     cout<<"R "<<"omega "<<R(2)<<" "<<omega<<endl;              
 	 // tempbasis->debug(R,omega, basisvals , currfunc); 
          tempbasis->calcSpinOrbit(R, omega, basisvals, currfunc);
	  ...
	  for(int m=0; m < nmo_list; m++) {
	    ...
	    if(eq_centers(mo)==ion){
	      ...
	      assert(basfunc<currfunc+1);
	      if(R(0) < cutoff(basfunc)){
		  newvals(m,0)+=magnification_factor*basisvals(basfunc,0)*kptfac(cen2);
              //    cout<<"hello basis so"<< d <<newvals(m,d,0)<<endl;
                  newvals(m,1)+=magnification_factor*basisvals(basfunc,1)*kptfac(cen2);
	      }
	    }
