                        ...
                        System * sys)
{
  ...
  //if(haskeyword(words, pos=startpos,"USE_SPIN"))
  //{ use_spin_sample=1; }
  //else use_spin_sample=0;
  ...
  if(readsection(words,pos, mowords, "ORBITALS"))  {
   if(use_spin_sample==1)
    error("You want to do spin sampling, but you supplied only real ORBITALS");
  ...
  if(readsection(words, pos, nspinstr, "NSPIN"))
  {
    ...
    if(use_spin_sample==1&&nelectrons(1)!=0)
    {  error("Spin sampling only supports single determinant calculation"); 
    }
