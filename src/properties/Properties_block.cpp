...
void Properties_block::storeToLog(string & indent, ostream & os,
  ...
  if(is_complex){
    print_array_sec(os, indent, "ikinetic_energy",avg(ikinetic) );
    print_array_sec(os, indent, "ikinetic_energyvar", var(ikinetic));
  }
...
void Properties_block::restoreFromLog(vector <string> & words) {
  ...
  if(is_complex){
    read_into_array(words, pos=0, avg, "ikinetic_energy", ikinetic);
    read_into_array(words, pos=0, var, "ikinetic_energyvar", ikinetic);
  }
...
void Properties_block::printBlockSummaryWithSpin(ostream & os) {
  int nwf=avg.GetDim(1);
  int naux=aux_energy.GetDim(0);
