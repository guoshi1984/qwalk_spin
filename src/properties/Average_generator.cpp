...
void Average_linear_delta_derivative::evaluate(Wavefunction_data * wfdata, Wavefunction * wf,
  ...
  if(unr){
    //possibly drift in like UNR sampling
    doublevar drift=vals.amp(0,1)*vals.amp(0,1)+vals.amp(0,2)*vals.amp(0,2)+vals.amp(0,3)*vals.amp(0,3); //stays the same even for complex WFs
    doublevar teff=tau;
    teff*=drift;
  ...
  //in the future we can change this to cvals if needed M.B.
  for(int d=0; d< ndim; d++){
    if(!derivatives.is_complex)
      avg.vals(d)=Jastrow_w2_inverse*derivatives.gradient(d)*gamma;
    else
      avg.vals(d)=Jastrow_w2_inverse*real(derivatives.cgradient(d))*gamma;  //real(\fhi_n/\Psi) d=0, -imag(\fhi_n/\Psi) d=1, etc... M.B.
  }
  avg.vals(ndim)=Jastrow_w2_inverse;
}
