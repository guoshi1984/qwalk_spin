...
void SO_HEG_sample::generateStorage(Sample_storage * & store)
{
...
//keep the same
void SO_HEG_sample::saveUpdate(int e, Sample_storage * store)
{
...
void SO_HEG_sample::restoreUpdate(int e, Sample_storage * store){
  for(int i=0; i<3; i++) {
 ...
 */
void SO_HEG_sample::updateEEDist() {
...
void SO_HEG_sample::init(System * sys) {
  assert(sys != NULL);
...
void SO_HEG_sample::randomGuess() //called by method::readcheck
{
  Array1 <doublevar> trialPos(3);
  doublevar trialOmega;
...
void SO_HEG_sample::setElectronPos(const int e,
...
void SO_HEG_sample::setElectronOmega(const int e, const doublevar & omega)
{ elecomega(e)=omega;
}

void SO_HEG_sample::translateElectron(const int e, const Array1 <doublevar> & trans) {
  Array1 <doublevar> temp(trans.GetDim(0));
  
  for(int d=0; d< 3; d++) 
  {   temp(d)=elecpos(e,d)+trans(d); }
  
  temp(3)=elecomega(e)+trans(3);
  Array1<int> nshift;
  ...
  elecomega(e)=temp(3);  //cout<<"trans omega"<<elecomega(0)<<endl;
...
void SO_HEG_sample::getElectronPos(const int e, Array1 <doublevar> & R)
{
    assert( R.GetDim(0) >= 3 );
    for(int i=0; i< 3; i++)
    {
...
void SO_HEG_sample::getElectronOmega(const int e, double & omega)
{   omega=elecomega(e);  
}
...
void SO_HEG_sample::rawOutput(ostream & os)
{
  os << "SO_HEG_sample\n";
  ...
  for(int e=0; e< nelectrons; e++)
  {
    ...
    os << setw(20)<< setprecision(12) << elecomega(e);
    os << "\n";
  }
}


void SO_HEG_sample::rawInput(istream & is)
{
  string text;
  is >> text;
