#include "SO_4D_Config_save_point.h"

void SO_4D_Config_save_point::write(ostream & os)
...
void SO_4D_Config_save_point::read(istream & is)
{ 
  string dummy;
  is >> dummy;
  assert(dummy=="nElec");
  ...
  for(int e=0; e< nelec; e++) 
  {
    ...
    electronomega(e).Resize(4);   
    for(int d=0; d< 3; d++) 
    {
      is >> electronpos(e)(d);
    }
    for(int d=0; d<4; d++)
    { is >> electronomega(e)(d);
    }
...
void SO_4D_Config_save_point::savePos(Sample_point * sample) {
...
void SO_4D_Config_save_point::restorePos(Sample_point * sample) 
{ 
  int nelectrons=sample->electronSize();
...
void SO_4D_Config_save_point::getPos(int e, Array1<doublevar> & r)
{  r=electronpos(e);
}

void SO_4D_Config_save_point::saveOmega(Sample_point * sample)
{ int nelectrons=sample->electronSize();
  if(electronomega.GetDim(0)!=nelectrons )
  { electronomega.Resize(nelectrons);
    for(int i=0; i< nelectrons; i++)
    { electronomega(i).Resize(4);}  
  }
...
void SO_4D_Config_save_point::restoreOmega(Sample_point * sample)
{ int nelectrons=sample->electronSize();
...
void SO_4D_Config_save_point::getOmega(int e, Array1<doublevar> & omega)
{ omega=electronomega(e);
}

void SO_4D_Config_save_point::mpiReceive(int node) {
#ifdef USE_MPI
  MPI_Status status;
  int nelectrons;
  ...
  for(int e=0; e< nelectrons; e++) {
    ...
    MPI_Recv(&electronomega(e), 4, MPI_DOUBLE,
        node, 0, MPI_Comm_grp, &status);
...
void SO_4D_Config_save_point::mpiSend(int node) {
#ifdef USE_MPI
  ...
  for(int e=0; e< nelectrons; e++) {
    MPI_Send(electronpos(e).v, 3, MPI_DOUBLE,
        node, 0, MPI_Comm_grp);
    MPI_Send(&electronomega(e), 4, MPI_DOUBLE,
        node, 0, MPI_Comm_grp);
  }
#endif
}
