...
int Molecular_system::showinfo(ostream & os)
{
  ions.showinfo(os);
  if(magnetic_field){
    os <<"Found Vector Potential (in symmetric gauge)"<<endl;
    os <<" A=(-y/2,x/2,0) * "<<Bbetaau/2<<" B_0 [B_0=4.7x10^9 G]"<<endl;
  }
  return 1;
}
                           ...
                           unsigned int & pos)
{
  ...
  magnetic_field=0;
  //read the Z component of mag. field in units of B_0=4.7x10^9 G;
  //to convert to atomic units we use 
  //1 a.u.=\hbar/(e*a_0^2)=2.35x10^9 G= B_0/2; 
  //so B = Bbeta* B_0 = 2 * Bbetaau in a.u
  //for atoms the effective Bbeta_Z is 1/Z^2 of that value
  //e.g. for He, Bbeta_Z=0.1 -> Bbetaau=2*0.1*4=0.8
  ;
  vector <string> magtxt;
  if(readsection(words, pos=0, magtxt, "MAGNETIC_FIELD")) { 
    if(magtxt.size()!=3) error("MAGNETIC_FIELD must have 3 components");
    if ( abs(atof(magtxt[0].c_str()))>0 || abs(atof(magtxt[1].c_str()))>0)
      error("only MAGNETIC_FIELD in Z direction is implemented yet");

    //take the z component only
    doublevar Bbeta=atof(magtxt[2].c_str());
    magnetic_field=1;
    Bbetaau=2*Bbeta;
  }
