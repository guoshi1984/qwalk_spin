...
void SO_4D_HEG_system::notify(change_type change, int n)
{
  switch(change)
  {
  case sample_static:
...
int SO_4D_HEG_system::generateSample(Sample_point *& samptr)
{
  assert(samptr==NULL);
  samptr=new SO_4D_HEG_sample;
  samptr->init(this);
  return 1;
}

int SO_4D_HEG_system::showinfo(ostream & os)
{
  const int ndim=3;
...
/*!
*/
int SO_4D_HEG_system::read(vector <string> & words,
                          unsigned int & pos)
{
  const int ndim=3;
  int startpos=pos;
  ...
  if(use_spin_sample==1&&nspin(1)!=0)
    error("SO_HEG only have one spin channel");
  ...
  */
  ...
  if(readsection(words, pos=0, interactiontxt, "INTERACTION")) {
    ...
    if( haskeyword(interactiontxt, pos=0, "GAUSS") ) {
      calcLocChoice=&SO_4D_HEG_system::calcLocGauss;
      eeModel=3;
  ...
  if(!readvalue(words,pos=0,so_mag,"SO_MAG")) so_mag=0;
  ...
  if ( eeModel == 1 ) 
   { if(dimension==2)  { setupEwald_2D(crossProduct); }
     else setupEwald(crossProduct);
   }
  
  if ( eeModel == 2 )
   { if(dimension==2) { setupTruncCoulomb_2D();}
     else setupTruncCoulomb();
   }
...
int SO_4D_HEG_system::setupEwald(Array2 <doublevar> & crossProduct) {
	...
	totgpt++;
        ...
        if(gsqrd > 1e-8) {  // throw away (0,0,0)
          gweighttemp(currgpt)=4.0 * pi*exp(-gsqrd/(4*alpha*alpha))
                               /(cellVolume*gsqrd); 
          if(gweighttemp(currgpt)>0.01)  
   cout<< " gweight " << gweighttemp(currgpt) <<" ig "<<ig<<" jg "<<jg<<" kg "<<kg<<" gsqrd "<< gsqrd <<endl;  
...
int SO_4D_HEG_system::setupEwald_2D(Array2 <doublevar> & crossProduct) {
  //----Set up reciprocal Ewald sum

  //const int ndim=3;
  smallestheight=1e99;   //!< smallest distance that spans the cell
  Area=latVec(0,0)*latVec(1,1);
  for(int i=0; i< 2; i++) {
    doublevar tempheight=0;
    doublevar length=0;
    for(int j=0; j< 2; j++) {
  ...
  // 5.0 chosen in PERIODIC system seems to be large enough; change
  // from 5.0 to 6.5 doubles the number of g-points 
  alpha=4/smallestheight;

  debug_write(cout, "alpha ", alpha, "\n");

  const int gmax=48;  //Could make this an option.
  const double qweight_cut=1e-18;

  ngpoints=(2*gmax+1)*(2*gmax+1);
  Array2 <doublevar> gpointtemp(ngpoints,3);
  Array1 <doublevar> gweighttemp(ngpoints);
  int currgpt=0;
  int totgpt=0;
  ...
  for(int ig=-gmax; ig <= gmax; ig++) {
    ...
    for(int jg=jgmin; jg <= gmax; jg++) {
        ...
        if(gsqrd > 1e-8) {  // throw away (0,0,0)
          gweighttemp(currgpt)=2.0 * pi*(erfcm(gsqrd/(2*alpha)))
                               /(Area*sqrt(gsqrd));
...
int SO_4D_HEG_system::setupTruncCoulomb() {
...
int SO_4D_HEG_system::setupTruncCoulomb_2D() {
  ...
  // backgr_trunc=1/V \int_{cell} 1/r
  // [ Mathematica helped with this one... ]
  doublevar backgr_trunc_single=3.52328543/L;
  backgr_trunc=totnelectrons*totnelectrons/2.0*backgr_trunc_single;
...
int SO_4D_HEG_system::enforcePbc(Array1 <doublevar> & pos, Array1 <int> & nshifted) {
...
doublevar SO_4D_HEG_system::calcLoc(Sample_point * sample)
{
...
doublevar SO_4D_HEG_system::calcLocEwald(Sample_point * sample)
{
  //cout << "Ewald local energy" << endl;
  doublevar ewalde;
  if(dimension==2) 
  {  ewalde=ewaldElectron_2D(sample);
...
doublevar SO_4D_HEG_system::calcLocTrunc(Sample_point * sample)
{
...
doublevar SO_4D_HEG_system::calcLocGauss(Sample_point * sample)
{
...
void SO_4D_HEG_system::constEwald() {
...
void SO_4D_HEG_system::constEwald_2D() {
  ...
  self_ee=-totnelectrons*(totnelectrons-1)*sqrt(pi)/(Area*alpha)
    -totnelectrons*sqrt(pi)/(Area*alpha)+totnelectrons*xi;
  cout<<" self_ee "<< self_ee <<endl;
  //Correct for the exchange-correlation false interaction with
  //the uniform electron gas number.  cexc was fitted to several
  //systems..
...
doublevar SO_4D_HEG_system::ewaldElectron(Sample_point * sample) {
  sample->updateEEDist();
...
doublevar SO_4D_HEG_system::ewaldElectron_2D(Sample_point * sample) {
  sample->updateEEDist();
  ...
  for(int e1=0; e1< totnelectrons; e1++) {
    for(int e2 =e1+1; e2 < totnelectrons; e2++) {
      sample->getEEDist(e1,e2, eidist);
      for(int d=0; d< 2; d++) r1(d)=eidist(d+2);
      ...
      for(int kk=-nlatvec; kk <=nlatvec; kk++) {
        for(int jj=-nlatvec; jj <=nlatvec; jj++) {
            ...
            for(int d=0; d< 2; d++) {
              r2(d)=r1(d)+kk*latVec(0,d)+jj*latVec(1,d);
            }
            doublevar r=sqrt(r2(0)*r2(0)+r2(1)*r2(1));
  ...
  {
   //  cout<< "elecpos "<<elecpos(i,0)<<" "<<elecpos(i,1)
     //        <<" "<<elecpos(i,2)<<endl;
  }
  doublevar elecElec_recip=0;
  for(int gpt=0; gpt < ngpoints; gpt++) {
    ...
    // NOTE: 1/2 from \sum_{e != e'} is cancelled with the fact that
    // we use only one g-point from g,-g pair
    elecElec_recip+=0.5*(sum_cos*sum_cos + sum_sin*sum_sin)*gweight(gpt);
  }
...
void SO_4D_HEG_system::calcSpinOrbit(Wavefunction_data * wfdata,
                          Sample_point * sample,
                          Wavefunction * wf,
                          Array1 <doublevar> & value)

{
  assert(value.GetDim(0)>=wf->nfunc());
  int nelectrons=sample->electronSize();
  int nwf=wf->nfunc();
  value=0;
  Array1<dcomplex> temp;
  temp.Resize(nwf);  
  for(int w=0; w<nwf; w++)
  {   
    for(int e=0; e<nelectrons; e++)
    {   
      wf->getSpinOrbit(wfdata, e, temp);
      //cout<<" temp "<< temp(0).real()<<endl;
      
      value(w)=value(w)+so_mag*temp(w).real();
    }
  }
