...
#include "SO_HEG_sample.h"
#include "HEG_system.h"
#include "qmc_io.h"
#include "MatrixAlgebra.h"

void SO_HEG_system::notify(change_type change, int n)
{
  switch(change)
  {
  case sample_static:
...
int SO_HEG_system::generateSample(Sample_point *& samptr)
{
  assert(samptr==NULL);
  samptr=new SO_HEG_sample;
  samptr->init(this);
  return 1;
}

int SO_HEG_system::showinfo(ostream & os)
{
  const int ndim=3;
...
/*!
*/
int SO_HEG_system::read(vector <string> & words,
                          unsigned int & pos)
{
  const int ndim=3;
  int startpos=pos;
  ...
  */
  ...
  if(readsection(words, pos=0, interactiontxt, "INTERACTION")) {
    ...
    if( haskeyword(interactiontxt, pos=0, "GAUSS") ) {
      calcLocChoice=&SO_HEG_system::calcLocGauss;
      eeModel=3;
...
int SO_HEG_system::setupEwald(Array2 <doublevar> & crossProduct) {
...
int SO_HEG_system::setupEwald_2D(Array2 <doublevar> & crossProduct) {
  //----Set up reciprocal Ewald sum
...
int SO_HEG_system::setupTruncCoulomb() {
...
int SO_HEG_system::setupTruncCoulomb_2D() {
  ...
  // backgr_trunc=1/V \int_{cell} 1/r
  // [ Mathematica helped with this one... ]
  doublevar backgr_trunc_single=3.5254944210/L;
  backgr_trunc=totnelectrons*totnelectrons/2.0*backgr_trunc_single;
...
int SO_HEG_system::enforcePbc(Array1 <doublevar> & pos, Array1 <int> & nshifted) {
...
doublevar SO_HEG_system::calcLoc(Sample_point * sample)
{
...
doublevar SO_HEG_system::calcLocEwald(Sample_point * sample)
{
...
doublevar SO_HEG_system::calcLocTrunc(Sample_point * sample)
{
...
doublevar SO_HEG_system::calcLocGauss(Sample_point * sample)
{
...
void SO_HEG_system::constEwald_2D() {
...
doublevar SO_HEG_system::ewaldElectron(Sample_point * sample) {
  sample->updateEEDist();
...
doublevar SO_HEG_system::ewaldElectron_2D(Sample_point * sample) {
  sample->updateEEDist();
...
void SO_HEG_system::calcSpinOrbit(Wavefunction_data * wfdata,
                          Sample_point * sample,
                          Wavefunction * wf,
